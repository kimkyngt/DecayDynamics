...
md"""
TimeHarp 260 [specification](https://www.picoquant.com/products/category/tcspc-and-time-tagging-modules/timeharp-260-tcspc-and-mcs-board-with-pcie-interface#specification). Important parameter to implement is `dead_time` = 2.5 ns for our "long range mode" option. If two or more photons arrive within `dead_time`, we loose the information. This would be called "pile up" effect. 
...
function get_summed_counts(sampled_data)
	return sum([sampled_data[ii].weights for ii in eachindex(sampled_data)])
end

# ╔═╡ b3865772-d3ac-49a1-be25-a485ba79ba0f
function pps_to_cps(sample::Histogram)
	cps_data = deepcopy(sample)
	cps_data.weights[cps_data.weights .> 1] .= 1
	return cps_data
end
...
begin
	...
	sampled_data = [fit(Histogram, rand(DoubleExp(tauP, tauD), photon_per_sync), sample_bins) for ii = 1:Nsample]
	
	plots_data = [plot(sampled_data[ii].edges[1][1:end-1], sampled_data[ii].weights, label=false, title="sync#$(ii)") for ii in 1:Nplot]
	
	push!(plots_data, plot(sample_bins[1:end-1], get_summed_counts(sampled_data), label=false, title="total"))
	
	plot(plots_data..., size=(800, 800), plot_title="Sampling. $(photon_per_sync) pps × $(Nsample) sync = $(photon_per_sync*Nsample) counts.")
end

# ╔═╡ 507b2f48-6dfd-4fbe-9edb-f78ce4de577c
cps_data = [pps_to_cps(sampled_data[ii]) for ii in eachindex(sampled_data)]
...
begin
	...
	# fit data
	@. model(x, p) = p[1]*(exp(-(x - p[5])/p[2]) - exp(-(x - p[5])/p[3])) + p[4]
	p0 = [maximum(y_raw), tauP*0.8, tauD*0.8, 0, 0]
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "fbe43642ba9747bdc6131418ad94f8c7a1f0f50e"
