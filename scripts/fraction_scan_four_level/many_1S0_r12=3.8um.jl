...
subfoldername = "fraction_scan_0.951S0_r12=2.6um"
...
distance = 1
for frac in fracs
        global filecount, distance
        exc_frac = 0.05*[frac, 1-frac] # Excitation fraction of level 1 and 3
        # Bfield = [0, 0, 0]
        # g_i = [1, 1, 1, 1]
        ...
        ψsingle_exc = sqrt(exc_frac[1])*spinup(SpinBasis(F_i[1])) ⊕ Ket(SpinBasis(F_i[2])) ⊕ sqrt(exc_frac[2])*spinup(SpinBasis(F_i[3])) ⊕ sqrt(0.95)*spinup(SpinBasis(F_i[4]))
