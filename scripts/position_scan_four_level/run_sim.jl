...
filecount = 1
for _z in range(0.05, 1, length=101)
        global filecount
        ...
        positions = [[0, 0, _z], [0, 0, 0]] # 1 = 2.6 um. 
        displacement = positions[1]-positions[2] # for saving
        tspan = range(0, 37, length=300)
...
for _x in range(0.05, 1, length=101)
        global filecount
        # Simulation parameters
        F_i = [2//1, 1//1, 1//1, 0//1] # two intermediate states, 3D1, 3P1, 3P0, 1S0
        Γ_kl = sparse(
        ...
        frac = 1
        exc_frac = [frac, 1-frac] # Excitation fraction of level 1 and 3
        positions = [[_x, 0, 0], [0, 0, 0]] # 1 = 2.6 um. 
        displacement = positions[1]-positions[2] # for saving
        tspan = range(0, 37, length=300)
        m_exc = F_i[1] -1  # excited state
        ψsingle_exc = ⊕([ii==1 ? Fm_state(F_i[ii], m_exc) : Ket(SpinBasis(F_i[ii])) for ii in eachindex(F_i)]...)
