...
function evolve_master(parameters::Dict; check_rates::Bool=false)
    ...
    H = sparse(sum(J(i, j, p, q, kl) * (dagger(dense(Σ_iq(i, p, F_i, kl[1], kl[2])))*Σ_iq(j, q, F_i, kl[1], kl[2])) for i in ptlindx for j in ptlindx for p in q_list for q in q_list for kl in transition_indx))
...
function plot_dynamics(result::Dict; kwargs...)
    ...
    fig = plot(fig1, fig3, fig2, fig4, layout=(2, 2), size=(1200, 800), plot_title="Spin: ($(latexify(F_i[1])), $(latexify(m_exc))) → $(latexify(F_i[2])), $(latexify(F_i[3])) → $(latexify(F_i[4])), "*L"\vec{r}_{12} = "*"$(round.(positions[1]-positions[2], digits=2)). Exc. frac: $(round(exc_frac[1], digits=3))", margins=5Plots.mm)
    return fig
end